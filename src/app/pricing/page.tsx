  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-12">
      ...
      <div className="text-center max-w-xl mx-auto space-y-3">
        ...
        <p className="text-xs text-slate-500">Choose a plan that fits your athletic goals. Save up to 35% with our annual billing options.</p>
      </div>

      {/* Monthly / Annual Toggle */}
      <div className="flex justify-center items-center space-x-3 mb-10 no-print">
        <span className={`text-xs font-bold ${billingPeriod === 'monthly' ? 'text-slate-800 dark:text-slate-150' : 'text-slate-400'}`}>Monthly</span>
        <button
          onClick={() => setBillingPeriod(billingPeriod === 'monthly' ? 'annual' : 'monthly')}
          className="w-14 h-7 rounded-full bg-slate-200 dark:bg-white/10 p-1 flex items-center transition-all cursor-pointer relative"
        >
          <div 
        ...
        <span className={`text-xs font-bold flex items-center ${billingPeriod === 'annual' ? 'text-slate-800 dark:text-slate-150' : 'text-slate-400'}`}>
          Annual Billing
          ...
          return (
            ...
            >
              ...
              <div className="space-y-4">
                <div>
                  <h3 className="text-lg font-black text-slate-800 dark:text-slate-100">{p.name}</h3>
                  <p className="text-xs text-slate-500 leading-tight mt-1">{p.desc}</p>
                </div>

                <div className="flex items-baseline space-x-1.5 py-4 border-t border-b border-slate-200/10">
                  <span className="text-4xl sm:text-5xl font-black text-slate-850 dark:text-slate-150">${currentPrice}</span>
                  <span className="text-xs text-slate-400 font-bold">/ {billingPeriod === 'monthly' ? 'month' : 'month, billed annually'}</span>
                </div>
                ...
                <button
                  ...
                  className={`w-full py-3.5 rounded-2xl font-black text-xs shadow-md transition-all cursor-pointer flex items-center justify-center space-x-1.5 active:scale-97 ${
                    p.popular
                      ...
                      : 'bg-slate-200/50 hover:bg-slate-200 dark:bg-white/5 dark:hover:bg-white/10 text-slate-700 dark:text-slate-200 border border-slate-350/15'
                  }`}
                >
                  <span>{p.cta}</span>
                </button>
