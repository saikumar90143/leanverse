      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="glass rounded-3xl p-6 border border-slate-200/10 space-y-4">
            <span className="text-xs font-black text-slate-400 uppercase tracking-widest block">Before & After Slider</span>
            ...
            >
              ...
              >
                <div className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-8 h-8 rounded-full bg-slate-900 border-2 border-white flex items-center justify-center text-white text-xs font-bold shadow-lg">
                  ↔
                </div>
              </div>
            </div>
            <p className="text-[10px] text-slate-500 text-center font-bold">Drag the center handle to compare visual muscle tone conditioning changes.</p>
          </div>

          {/* Measurements grid */}
          <div className="glass rounded-3xl p-6 border border-slate-200/10 space-y-4">
            <span className="text-xs font-black text-slate-400 uppercase tracking-widest block">Tape Circumference Measurements (cm)</span>
            
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div className="p-3 bg-slate-100/50 dark:bg-white/5 border border-slate-200/5 rounded-2xl">
                <span className="text-[10px] text-slate-400 font-extrabold block">Chest</span>
                <input 
                  type="number" min="0" 
                  value={chest} 
                  onChange={(e) => setChest(parseInt(e.target.value) || 0)}
                  className="bg-transparent border-0 font-black text-lg text-slate-800 dark:text-slate-100 focus:ring-0 focus:outline-none w-full mt-1"
                />
              </div>

              <div className="p-3 bg-slate-100/50 dark:bg-white/5 border border-slate-200/5 rounded-2xl">
                <span className="text-[10px] text-slate-400 font-extrabold block">Waist</span>
                <input 
                  type="number" min="0" 
                  value={waist} 
              ...
              <div className="p-3 bg-slate-100/50 dark:bg-white/5 border border-slate-200/5 rounded-2xl">
                <span className="text-[10px] text-slate-400 font-extrabold block">Biceps</span>
                <input 
                  type="number" min="0" 
                  value={biceps} 
              ...
              <div className="p-3 bg-slate-100/50 dark:bg-white/5 border border-slate-200/5 rounded-2xl">
                <span className="text-[10px] text-slate-400 font-extrabold block">Hips</span>
                <input 
                  type="number" min="0" 
                  value={hips} 
        ...
        <div className="lg:col-span-5 space-y-6">
          {/* Line Chart */}
          <div className="glass rounded-3xl p-6 border border-slate-200/10 space-y-4">
            <span className="text-xs font-black text-slate-400 uppercase tracking-widest block">Weight progress (kg)</span>
            ...
            {/* Quick logger */}
            <form onSubmit={handleWeightSubmit} className="flex space-x-2 pt-4 border-t border-slate-200/10">
              <input
                type="text"
                placeholder="Log Weight kg"
                value={newWeight}
                onChange={(e) => setNewWeight(e.target.value)}
                className="flex-1 bg-slate-100/50 dark:bg-white/5 border border-slate-350/20 dark:border-white/10 rounded-xl px-3.5 py-2.5 text-xs focus:outline-none focus:border-emerald-500 font-bold text-slate-800 dark:text-slate-100"
              />
              <button
                type="submit"
          ...
          <div className="glass rounded-3xl p-6 border border-slate-200/10 space-y-4">
            <span className="text-xs font-black text-slate-400 uppercase tracking-widest block">Recorded Weight logs</span>
            <div className="space-y-2.5 overflow-y-auto max-h-[180px] pr-1 no-scrollbar">
              {weightLogs.slice().reverse().map((log, index) => (
                <div key={index} className="flex justify-between items-center py-2.5 border-b border-slate-200/10 last:border-0 text-sm font-semibold">
                  <div className="flex items-center space-x-2 text-slate-500">
                    <Calendar className="w-4 h-4" />
                    <span>{log.date}</span>
                  </div>
                  <span className="text-slate-850 dark:text-slate-200 font-black">{log.weight} kg</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="glass rounded-3xl p-6 border border-slate-200/10 space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-xs font-black text-slate-400 uppercase tracking-widest block">Estimated 1RM Analytics (kg)</span>
            ...
            <p className="text-[10px] text-slate-500 text-center font-bold">Estimated 1RM uses the Epley formula based on your logged working sets.</p>
          </div>
        </div>
      </div>
    </div>
