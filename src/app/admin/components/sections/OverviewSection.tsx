...
function KpiCard({ label, value, sub, icon: Icon, color, sparkData }: {
  ...
  return (
    <div className="glass rounded-2xl p-5 border border-slate-200/10 dark:border-white/5 relative overflow-hidden flex flex-col justify-between min-h-[110px]">
...
export default function OverviewSection() {
  ...
  const growthNums = stats?.charts?.userGrowth?.map((d) => d.count) || [2, 4, 3, 7, 5, 9, 8];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-4">
        ...
        <KpiCard label="Active Today" value={kpi?.activeToday?.toLocaleString() ?? '—'} sub="Unique sessions" icon={Activity} color="bg-cyan-500/10 text-cyan-500" sparkData={[...growthNums].reverse()} />
        ...
        <KpiCard label="Workout Plans" value={kpi?.totalWorkoutPlans?.toLocaleString() ?? '—'} icon={Dumbbell} color="bg-rose-500/10 text-rose-500" sparkData={growthNums} />
        <KpiCard label="Diet Plans" value={kpi?.totalDietPlans?.toLocaleString() ?? '—'} icon={Apple} color="bg-green-500/10 text-green-500" />
        <KpiCard label="Monthly Revenue" value={`₹${(kpi?.monthlyRevenue ?? 0).toLocaleString()}`} icon={DollarSign} color="bg-emerald-500/10 text-emerald-500" sparkData={[300,450,600,420,750,900,kpi?.monthlyRevenue ?? 800]} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* User Growth Chart */}
        <div className="glass rounded-2xl p-5 border border-slate-200/10 dark:border-white/5">
          <h3 className="text-xs font-black text-slate-500 dark:text-zinc-400 uppercase tracking-widest mb-4">User Growth (Last 7 Days)</h3>
          <div className="flex items-end gap-1.5 h-28">
            {(stats?.charts?.userGrowth ?? growthNums.map((c, i) => ({ date: `Day ${i}`, count: c }))).map((d, i) => {
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <div
                    ...
                    style={{ height: `${pct}%` }}
                  />
                  <span className="text-[8px] text-slate-400 dark:text-zinc-600">{((d as any).date ?? '').slice(5)}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="glass rounded-2xl p-5 border border-slate-200/10 dark:border-white/5">
          <h3 className="text-xs font-black text-slate-500 dark:text-zinc-400 uppercase tracking-widest mb-4">Blog Views by Category</h3>
          <div className="space-y-3">
            {(stats?.charts?.blogByCategory?.length
              ? stats?.charts?.blogByCategory
              : [
                  { _id: 'Indian Diet', count: 4, views: 1200 },
                  { _id: 'Gym Workout', count: 6, views: 980 },
                  { _id: 'Supplements', count: 2, views: 540 },
                  { _id: 'Home Workout', count: 3, views: 430 },
                ]
            ).slice(0, 5).map((cat: any, i) => {
              const maxViews = Math.max(...(stats?.charts?.blogByCategory?.map(c => c.views) ?? [1200]), 1);
              ...
              return (
                <div key={i} className="space-y-1">
                  <div className="flex justify-between text-[10px] font-bold">
                    <span className="text-slate-600 dark:text-zinc-300">{cat._id}</span>
                    <span className="text-slate-400">{cat.views?.toLocaleString() ?? 0} views</span>
                  </div>
                  <div className="h-2 bg-slate-200/40 dark:bg-white/5 rounded-full overflow-hidden">
      ...
      <div className="glass rounded-2xl p-5 border border-slate-200/10 dark:border-white/5">
        <h3 className="text-xs font-black text-slate-500 dark:text-zinc-400 uppercase tracking-widest mb-4">Recent Registrations</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-slate-200/10 dark:border-white/5">
                <th className="text-left pb-2 font-black text-slate-400 dark:text-zinc-500">User</th>
                <th className="text-left pb-2 font-black text-slate-400 dark:text-zinc-500">Email</th>
                <th className="text-left pb-2 font-black text-slate-400 dark:text-zinc-500">Tier</th>
                <th className="text-left pb-2 font-black text-slate-400 dark:text-zinc-500">Streak</th>
                <th className="text-left pb-2 font-black text-slate-400 dark:text-zinc-500">Joined</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200/5 dark:divide-white/5">
              {(stats?.recentUsers?.length
                ? stats.recentUsers
                : [
                    { _id: '1', name: 'Rohan Sharma', email: 'rohan@demo.com', tier: 'premium', streak: 12, createdAt: new Date().toISOString() },
                    { _id: '2', name: 'Priya Verma', email: 'priya@demo.com', tier: 'free', streak: 3, createdAt: new Date().toISOString() },
                    { _id: '3', name: 'Arjun Singh', email: 'arjun@demo.com', tier: 'pro', streak: 28, createdAt: new Date().toISOString() },
                  ]
              ).map((u) => (
                <tr key={(u as any)._id} className="hover:bg-slate-50/30 dark:hover:bg-white/3">
                  <td className="py-2.5 font-bold text-slate-700 dark:text-zinc-200">{u.name}</td>
