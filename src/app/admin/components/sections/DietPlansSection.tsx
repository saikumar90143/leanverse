...
export default function DietPlansSection() {
 ...
 <h2 className="text-xl font-black text-foreground">Diet Plans</h2>
 ...
 <div className="relative w-full max-w-4xl bg-card rounded-3xl shadow-2xl border border-border/20 dark:border-border overflow-hidden max-h-[90vh] flex flex-col">
 <div className="flex items-center justify-between p-5 border-b border-border/10 dark:border-border bg-background dark:bg-card/2">
 <h3 className="font-black text-foreground">{editingPlan._id ? 'Edit Diet Plan' : 'Diet Plan Builder'}</h3>
 <button onClick={() => setShowPlanModal(false)}><X className="w-5 h-5 text-muted" /></button>
 </div>
 
 <div className="flex-1 overflow-y-auto p-5 grid grid-cols-1 md:grid-cols-3 gap-6">
 {/* Left Col: Meta Info */}
 <div className="space-y-4">
 <div className="glass p-4 rounded-2xl border border-border/20 dark:border-border space-y-4">
 <div>
 <label className="text-[10px] font-black text-muted uppercase block mb-1">Plan Name *</label>
 <input value={editingPlan.name} onChange={(e) => setEditingPlan({ ...editingPlan, name: e.target.value })}
 className="w-full px-3 py-2 bg-card border border-border/20 dark:border-border rounded-xl text-sm focus:outline-none focus:border-emerald-500" placeholder="e.g. 30-Day Shred" />
 </div>
 <div>
 <label className="text-[10px] font-black text-muted uppercase block mb-1">Goal</label>
 ...
 <div>
 <label className="text-[10px] font-black text-muted uppercase block mb-1">Duration (Days)</label>
 ...
 <div>
 <label className="text-[10px] font-black text-muted uppercase block mb-1">Description</label>
 <textarea value={editingPlan.description} onChange={(e) => setEditingPlan({ ...editingPlan, description: e.target.value })} rows={3}
 className="w-full px-3 py-2 bg-card border border-border/20 dark:border-border rounded-xl text-sm focus:outline-none focus:border-emerald-500 resize-none" placeholder="Short description for users..." />
 </div>
 </div>

 {/* Live Macros */}
 <div className="glass p-4 rounded-2xl border border-border/20 dark:border-border">
 <h4 className="text-[10px] font-black text-muted uppercase mb-3">Live Plan Macros</h4>
 <div className="text-center mb-3">
 <span className="text-3xl font-black text-foreground">{currentMacros.cal}</span>
 <span className="text-xs font-bold text-muted ml-1">kcal</span>
 </div>
 <div className="grid grid-cols-3 gap-2 text-center text-xs">
 <div className="bg-rose-500/10 rounded-lg py-1.5"><p className="font-bold text-rose-500">{currentMacros.pro}g</p><p className="text-[9px] text-muted">Protein</p></div>
 <div className="bg-cyan-500/10 rounded-lg py-1.5"><p className="font-bold text-cyan-500">{currentMacros.car}g</p><p className="text-[9px] text-muted">Carbs</p></div>
 <div className="bg-amber-500/10 rounded-lg py-1.5"><p className="font-bold text-amber-500">{currentMacros.fat}g</p><p className="text-[9px] text-muted">Fat</p></div>
 </div>
 </div>
 </div>
 ...
 <div key={meal.name} className="glass rounded-2xl border border-border/20 dark:border-border overflow-hidden">
 <div className="flex items-center justify-between p-3 border-b border-border/10 dark:border-border bg-background dark:bg-card/2">
 <h4 className="font-bold text-sm text-foreground ">{meal.name}</h4>
 ...
 <div className="p-3">
 {meal.foods.length === 0 ? (
 <p className="text-xs text-muted text-center py-2">No foods added.</p>
 ) : (
 ...
 if (!fi) return null;
 return (
 <div key={i} className="flex items-center justify-between bg-card p-2 rounded-xl border border-slate-100 dark:border-border">
 <div className="flex items-center gap-2">
 <span className="text-xl">{fi.emoji}</span>
 <div>
 <p className="text-xs font-bold text-foreground ">{fi.name}</p>
 <p className="text-[10px] text-muted">{fi.calories} cal | {fi.protein}P {fi.carbs}C {fi.fat}F (per {fi.servingUnit})</p>
 </div>
 </div>
 <div className="flex items-center gap-2">
 <div className="flex items-center bg-secondary rounded-lg px-2 py-1">
 <span className="text-[10px] text-muted mr-1">Qty</span>
 ...
 className="w-10 bg-transparent text-xs font-bold text-center focus:outline-none dark:text-foreground" />
 </div>
 ...
 <div className="p-5 border-t border-border/10 dark:border-border flex justify-end gap-3 bg-background dark:bg-card/2">
 <button onClick={() => setShowPlanModal(false)} className="px-4 py-2.5 rounded-xl text-xs font-bold bg-card border border-border text-muted">Cancel</button>
 ...
 <div className="relative w-full max-w-lg bg-card rounded-3xl shadow-2xl border border-border/20 dark:border-border overflow-hidden flex flex-col h-[500px]">
 <div className="p-4 border-b border-border/10 dark:border-border flex items-center gap-3">
 <Apple className="w-5 h-5 text-emerald-500" />
 <h3 className="font-black text-foreground flex-1">Add to {showFoodSearch}</h3>
 <button onClick={() => setShowFoodSearch(null)}><X className="w-5 h-5 text-muted" /></button>
 </div>
 <div className="p-3 border-b border-border/10 dark:border-border">
 ...
 className="w-full pl-9 pr-4 py-2 bg-background dark:bg-card/5 border border-border/20 dark:border-border rounded-xl text-sm focus:outline-none focus:border-emerald-500" />
 </div>
 </div>
 <div className="flex-1 overflow-y-auto p-2">
 {filteredFoods.length === 0 ? (
 <p className="text-center text-sm text-muted py-10">No foods found. Check your Food Database.</p>
 ) : (
 filteredFoods.map(f => (
 <button key={f._id} onClick={() => handleAddFoodToMeal(f)} className="w-full flex items-center justify-between p-3 rounded-xl hover:bg-background dark:hover:bg-card/5 transition-colors text-left group">
 <div className="flex items-center gap-3">
 <span className="text-2xl">{f.emoji}</span>
 <div>
 <p className="text-sm font-bold text-foreground ">{f.name}</p>
 <p className="text-[10px] text-muted">{f.calories} cal | {f.protein}P {f.carbs}C {f.fat}F ({f.servingUnit})</p>
 </div>
 </div>
