    ...
    <div className="space-y-6">
      ...
      <div className="glass rounded-2xl p-6 border border-slate-200/10 dark:border-white/5 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <h3 className="font-black text-slate-800 dark:text-white text-sm">Send Push Notification</h3>
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2">
            <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Notification Title</label>
          ...
          <div className="col-span-2">
            <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Message Body</label>
          ...
          <div>
            <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Target Segment</label>
          ...
          <div>
            <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">When</label>
            <div className="flex gap-2">
              {['now', 'schedule'].map(opt => (
                <button key={opt} onClick={() => setSchedule(opt)}
                  className={`flex-1 py-2.5 rounded-xl text-xs font-bold border transition-all ${schedule === opt ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-slate-100/50 dark:bg-white/5 text-slate-400 border-slate-200/20 dark:border-white/10'}`}>
          ...
          {schedule === 'schedule' && (
            <div>
              <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Schedule Time</label>
      ...
      <div className="glass rounded-2xl p-6 border border-slate-200/10 dark:border-white/5 space-y-4">
        <h3 className="font-black text-slate-800 dark:text-white text-sm">Recent Notifications</h3>
        <div className="space-y-3">
          {sentNotifications.map((n, i) => {
            ...
            return (
              <div key={i} className="p-4 bg-slate-100/30 dark:bg-white/3 rounded-2xl border border-slate-200/10 dark:border-white/5">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <p className="font-bold text-slate-700 dark:text-zinc-200">{n.title}</p>
                    <p className="text-xs text-slate-400 mt-0.5">{n.body}</p>
                    <p className="text-[10px] text-slate-400 mt-1">→ {n.segment} · {n.time}</p>
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-3 mt-3">
                  {[
                    { label: 'Delivered', value: n.delivered.toLocaleString(), color: 'text-slate-700 dark:text-zinc-200' },
                    ...
                    <div key={s.label} className="bg-white/50 dark:bg-white/5 rounded-xl p-2 text-center">
