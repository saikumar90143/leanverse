...
export default function ExercisesSection() {
  ...
  return (
    <div className="space-y-5">
      ...
      <p className="text-xs text-slate-400 font-bold">{total.toLocaleString()} exercises in database</p>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-slate-200/10 dark:border-white/5 bg-slate-50/50 dark:bg-white/2">
                <th className="text-left px-4 py-3 font-black text-slate-400">Exercise</th>
                <th className="text-left px-4 py-3 font-black text-slate-400">Muscle</th>
                <th className="text-left px-4 py-3 font-black text-slate-400">Equipment</th>
                <th className="text-left px-4 py-3 font-black text-slate-400">Difficulty</th>
                <th className="text-left px-4 py-3 font-black text-slate-400">Cal/min</th>
                <th className="text-left px-4 py-3 font-black text-slate-400">Uses</th>
            ...
            <tbody className="divide-y divide-slate-200/5 dark:divide-white/5">
              {loading ? (
                ...
                <tr key={ex._id} className="hover:bg-slate-50/30 dark:hover:bg-white/3">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      {ex.imageUrl ? (
                        <img src={ex.imageUrl} alt={ex.name} className="w-8 h-8 rounded-lg object-cover" />
                      ) : (
                        <div className="w-8 h-8 rounded-lg bg-slate-200 dark:bg-white/5 flex items-center justify-center">
                          <span className="text-[10px] font-bold text-slate-400">{ex.name.charAt(0)}</span>
                        </div>
                      )}
                      <span className="font-bold text-slate-700 dark:text-zinc-200">{ex.name}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-slate-500">{ex.muscleGroup}</td>
                  <td className="px-4 py-3 text-slate-500">{ex.equipment}</td>
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-1">
                      {ex.difficulty.map((diff) => (
                        <span key={diff} className={`px-2 py-0.5 rounded-full text-[9px] font-black ${diffColors[diff] || 'bg-slate-500/10 text-slate-500'}`}>
                          {diff}
                        </span>
                      ))}
                    </div>
                  </td>
                  <td className="px-4 py-3 text-slate-500">{ex.caloriesPerMinute}</td>
                  <td className="px-4 py-3 text-slate-500">{ex.usageCount}</td>
                  <td className="px-4 py-3">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-2xl bg-white dark:bg-zinc-900 rounded-3xl shadow-2xl border border-slate-200/20 dark:border-white/10 overflow-hidden max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between p-6 border-b border-slate-200/10 dark:border-white/5">
              <h3 className="font-black text-slate-800 dark:text-white">{editing._id ? 'Edit Exercise' : 'Add Exercise'}</h3>
              <button onClick={() => setShowModal(false)}><X className="w-5 h-5 text-slate-400" /></button>
            </div>
            <div className="overflow-y-auto p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Exercise Name *</label>
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Difficulty</label>
                  <div className="flex flex-wrap gap-2">
                    {DIFFICULTIES.map(d => (
                      <label key={d} className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300 cursor-pointer">
                        <input
                          type="checkbox"
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Target Sets (Min - Max)</label>
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Target Reps (Min - Max)</label>
                  <div className="flex gap-2">
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Rest Period (Seconds)</label>
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Calories/Minute</label>
                ...
                <div className="col-span-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Exercise Image</label>
                  <div className="flex items-center gap-4 p-3 bg-slate-50 dark:bg-white/5 border border-slate-200/20 dark:border-white/10 rounded-xl">
                    {editing.imageUrl ? (
                      <div className="relative group">
                        <img src={editing.imageUrl} alt="Preview" className="w-16 h-16 rounded-lg object-cover bg-white" />
                        <button 
                    ...
                    <div className="flex-1">
                      <input 
                        ...
                        className="w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-emerald-500 file:text-white hover:file:bg-emerald-600 cursor-pointer"
                      />
                      <p className="text-[10px] text-slate-400 mt-1">Recommended: .webp or transparent .webp</p>
                    </div>
                  </div>
                </div>
                <div className="col-span-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Instructions (One step per line)</label>
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Home Version</label>
                ...
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Gym Version</label>
