...
export default function SubscriptionsSection() {
 ...
 {plans.map((plan) => (
 <div key={plan.name} className="glass rounded-2xl p-6 border border-border/10 dark:border-border space-y-4">
 <div className="flex items-start justify-between">
 <div>
 <h3 className="font-black text-foreground">{plan.name}</h3>
 <p className="text-xs text-muted">Per {plan.interval || 'month'}</p>
 </div>
 ...
 {[
 { label: 'Subscribers', value: (plan.subscribers || 0).toLocaleString(), color: 'text-foreground ' },
 ...
 <div key={m.label} className="bg-secondary/50 dark:bg-card/3 rounded-xl p-3">
 ...
 <div className="h-1.5 bg-secondary/40 dark:bg-card/5 rounded-full overflow-hidden">
 ...
 <p className="text-[10px] text-muted text-right">{totalSubscribers ? (((plan.subscribers || 0) / totalSubscribers) * 100).toFixed(1) : 0}% of all subscribers</p>
 </div>
 ))}
 </div>
 </div>
