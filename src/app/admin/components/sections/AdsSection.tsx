  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {adMetrics.map((m) => (
          <div key={m.label} className="glass rounded-2xl p-4 border border-slate-200/10 dark:border-white/5 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-black text-slate-800 dark:text-white">{m.value}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="glass rounded-2xl p-6 border border-slate-200/10 dark:border-white/5">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-black text-slate-800 dark:text-white">Global AdSense Status</h3>
            <p className="text-xs text-slate-400 mt-0.5">Master toggle — disables all ads across the entire platform</p>
          </div>
      ...
      <div className="glass rounded-2xl p-6 border border-slate-200/10 dark:border-white/5 space-y-4">
        <h3 className="font-black text-slate-800 dark:text-white text-sm">Placement Control</h3>
          ...
          {placements.map(({ key, label, desc }) => {
            ...
            return (
              <div key={key} onClick={() => setSettings(s => ({ ...s, adPlacements: { ...s.adPlacements, [key]: !enabled } }))}
                className={`flex items-center justify-between p-4 rounded-2xl border cursor-pointer transition-all ${enabled ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-slate-100/30 dark:bg-white/3 border-slate-200/10 dark:border-white/5'}`}>
                <div>
                  <p className={`font-bold text-sm ${enabled ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-600 dark:text-zinc-300'}`}>{label}</p>
                  <p className="text-[10px] text-slate-400">{desc}</p>
                </div>
                {enabled ? <ToggleRight className="w-8 h-8 text-emerald-500 shrink-0" /> : <ToggleLeft className="w-8 h-8 text-slate-400 shrink-0" />}
              </div>
            );
          })}
        </div>
      ...
      {/* AdSense ID */}
      <div className="glass rounded-2xl p-6 border border-slate-200/10 dark:border-white/5 space-y-3">
        <h3 className="font-black text-slate-800 dark:text-white text-sm">AdSense Publisher ID</h3>
        <input
          value={settings.adsenseId}
          onChange={(e) => setSettings(s => ({ ...s, adsenseId: e.target.value }))}
          placeholder="ca-pub-XXXXXXXXXXXXXXXX"
          className="w-full px-4 py-3 bg-slate-100/50 dark:bg-white/5 border border-slate-200/20 dark:border-white/10 rounded-xl text-sm font-mono focus:outline-none focus:border-emerald-500"
        />
        <p className="text-[10px] text-slate-400">This ID is used across all ad placements. Find it in your AdSense account settings.</p>
      </div>
