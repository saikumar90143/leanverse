    ...
    <div className="space-y-6">
      ...
      <div className="glass rounded-2xl p-6 border border-slate-200/10 dark:border-white/5 space-y-4">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h3 className="font-black text-slate-800 dark:text-white text-sm">Role Management</h3>
            <p className="text-[10px] text-slate-400">Define what each role can access</p>
          </div>
        </div>
            ...
            <div key={role.name} className={`p-4 rounded-2xl border ${role.color}`}>
              ...
              <div className="flex flex-wrap gap-1">
                {role.permissions.map(p => (
                  <span key={p} className="text-[9px] font-black bg-black/10 dark:bg-white/10 px-1.5 py-0.5 rounded-md">{p}</span>
                ))}
              </div>
            </div>
          ))}
      ...
      <div className="glass rounded-2xl p-6 border border-slate-200/10 dark:border-white/5 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h3 className="font-black text-slate-800 dark:text-white text-sm">API Keys</h3>
          </div>
        ...
        <div className="space-y-3">
          {apiKeys.map((ak) => (
            <div key={ak.name} className="flex items-center justify-between p-4 bg-slate-100/50 dark:bg-white/3 rounded-2xl border border-slate-200/10 dark:border-white/5">
              <div>
                <p className="font-bold text-slate-700 dark:text-zinc-200 text-sm">{ak.name}</p>
                <p className="text-xs font-mono text-slate-400 mt-0.5">{ak.key}</p>
                <p className="text-[10px] text-slate-400 mt-0.5">Last used {ak.last}</p>
              </div>
              <button onClick={() => copyKey(ak.key)}
                className={`p-2 rounded-xl transition-all ${copiedKey === ak.key ? 'bg-emerald-500/10 text-emerald-500' : 'bg-slate-200/50 dark:bg-white/5 text-slate-500 hover:text-slate-700 dark:hover:text-zinc-200'}`}>
      ...
      <div className="glass rounded-2xl p-6 border border-slate-200/10 dark:border-white/5 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <h3 className="font-black text-slate-800 dark:text-white text-sm">Audit Log</h3>
        </div>
        <div className="space-y-2">
          {loading ? (
            <div className="p-4 text-center text-xs text-slate-400">Loading audit logs...</div>
          ) : auditLogs.length === 0 ? (
            <div className="p-4 text-center text-xs text-slate-400">No audit logs found</div>
          ) : auditLogs.map((log, i) => (
            <div key={i} className="flex items-center gap-3 p-3 bg-slate-100/30 dark:bg-white/3 rounded-xl border border-slate-200/10 dark:border-white/5">
              <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-xs font-bold text-slate-700 dark:text-zinc-200">{log.action}</p>
                <p className="text-[10px] text-slate-400 truncate">{log.adminEmail} → {log.resource}</p>
              </div>
              <span className="text-[10px] text-slate-400 shrink-0">{new Date(log.createdAt).toLocaleDateString()}</span>
            </div>
          ))}
        </div>
      </div>
