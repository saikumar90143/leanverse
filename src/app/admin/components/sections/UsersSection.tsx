...
export default function UsersSection() {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-52">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 bg-white/50 dark:bg-white/5 border border-slate-200/20 dark:border-white/10 rounded-xl text-xs font-medium focus:outline-none focus:border-emerald-500 transition-colors"
          />
        </div>
        <select
          value={tierFilter}
          onChange={(e) => { setTierFilter(e.target.value); setPage(1); }}
          className="px-3 py-2.5 bg-white/50 dark:bg-white/5 border border-slate-200/20 dark:border-white/10 rounded-xl text-xs font-bold focus:outline-none focus:border-emerald-500"
        >
      ...
      {/* Stats row */}
      <div className="text-xs text-slate-400 dark:text-zinc-500 font-bold">
      ...
      <div className="glass rounded-2xl border border-slate-200/10 dark:border-white/5 overflow-hidden">
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-slate-200/10 dark:border-white/5 bg-slate-50/50 dark:bg-white/2">
                <th className="text-left px-4 py-3 font-black text-slate-400 dark:text-zinc-500">User</th>
                <th className="text-left px-4 py-3 font-black text-slate-400 dark:text-zinc-500">Role</th>
                <th className="text-left px-4 py-3 font-black text-slate-400 dark:text-zinc-500">Tier</th>
                <th className="text-left px-4 py-3 font-black text-slate-400 dark:text-zinc-500">Streak</th>
                <th className="text-left px-4 py-3 font-black text-slate-400 dark:text-zinc-500">Last Active</th>
                <th className="text-left px-4 py-3 font-black text-slate-400 dark:text-zinc-500">Joined</th>
            ...
            <tbody className="divide-y divide-slate-200/5 dark:divide-white/5">
              {loading ? (
                ...
                <tr key={u._id} className="hover:bg-slate-50/30 dark:hover:bg-white/3 transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <p className="font-bold text-slate-700 dark:text-zinc-200">{u.name}</p>
                  ...
                  <td className="px-4 py-3 text-slate-400">{u.lastActive ? new Date(u.lastActive).toLocaleDateString() : '—'}</td>
                  <td className="px-4 py-3 text-slate-400">{new Date(u.createdAt).toLocaleDateString()}</td>
                  <td className="px-4 py-3">
                    <button
                      onClick={() => setSelected(u)}
                      className="px-3 py-1.5 bg-slate-100 dark:bg-white/5 text-slate-500 dark:text-zinc-400 rounded-lg hover:bg-emerald-500/10 hover:text-emerald-500 transition-colors font-bold text-[10px]"
                    >
                      Manage
                    </button>
                  </td>
        ...
        {/* Pagination */}
        <div className="flex items-center justify-between px-4 py-3 border-t border-slate-200/10 dark:border-white/5">
          <span className="text-[10px] text-slate-400 font-bold">Page {page} of {pages}</span>
          <div className="flex gap-2">
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="p-1.5 rounded-lg bg-slate-100 dark:bg-white/5 text-slate-500 disabled:opacity-30">
              <ChevronLeft className="w-4 h-4" />
            </button>
            <button onClick={() => setPage(p => Math.min(pages, p + 1))} disabled={page === pages} className="p-1.5 rounded-lg bg-slate-100 dark:bg-white/5 text-slate-500 disabled:opacity-30">
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-sm bg-white dark:bg-zinc-900 h-full overflow-y-auto shadow-2xl p-6 space-y-5 flex flex-col">
            <div className="flex items-center justify-between">
              <h3 className="font-black text-slate-800 dark:text-white text-sm">Manage User</h3>
              <button onClick={() => setSelected(null)} className="text-slate-400 hover:text-slate-700 dark:hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>

            <div className="glass rounded-2xl p-4 border border-slate-200/10 dark:border-white/5">
              ...
              <p className="font-black text-slate-800 dark:text-white text-sm">{selected.name}</p>
            ...
            <div className="space-y-2 flex-1">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Change Tier</p>
              {['free', 'premium', 'pro'].map(t => (
                <button key={t} onClick={() => doAction('setTier', t)} disabled={actionLoading}
                  className={`w-full py-2.5 rounded-xl text-xs font-bold border transition-all ${selected.tier === t ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-slate-100/50 dark:bg-white/5 text-slate-500 border-slate-200/20 dark:border-white/10 hover:border-emerald-500/30'}`}>
                  {t.toUpperCase()} {selected.tier === t ? '✓' : ''}
                </button>
              ))}

              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-4">Actions</p>
