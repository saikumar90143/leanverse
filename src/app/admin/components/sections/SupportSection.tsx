...
const statusConfig: Record<string, { label: string; icon: React.ElementType; color: string }> = {
 ...
 closed: { label: 'Closed', icon: XCircle, color: 'bg-background0/10 text-muted border-slate-500/20' },
};
...
export default function SupportSection() {
 ...
 className={`glass rounded-2xl p-4 border transition-all text-left ${statusFilter === key ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-border/10 dark:border-border'}`}>
 <div className="flex items-center gap-2 mb-2">
 <Icon className="w-4 h-4 text-muted" />
 <span className="text-[10px] font-black text-muted uppercase">{label}</span>
 </div>
 <p className="text-2xl font-black text-foreground">{counts[key as keyof typeof counts]}</p>
 </button>
 ))}
 </div>
 ...
 <th className="text-left px-4 py-3 font-black text-muted">Subject</th>
 <th className="text-left px-4 py-3 font-black text-muted">Category</th>
 <th className="text-left px-4 py-3 font-black text-muted">Priority</th>
 ...
 return (
 <tr key={t._id} className="hover:bg-background/30 dark:hover:bg-card/3">
 <td className="px-4 py-3">
 <p className="font-bold text-foreground ">{t.userName}</p>
 <p className="text-muted">{t.userEmail}</p>
 </td>
 <td className="px-4 py-3 font-medium text-muted max-w-40 truncate">{t.subject}</td>
 ...
 className="px-3 py-1.5 bg-secondary dark:bg-card/5 text-muted rounded-lg hover:bg-blue-500/10 hover:text-blue-500 transition-colors font-bold text-[10px]">
 View
 </button>
 </td>
 </tr>
 ...
 <div className="relative w-full max-w-sm bg-card h-full overflow-y-auto shadow-2xl p-6 space-y-5">
 <div className="flex items-center justify-between">
 <h3 className="font-black text-foreground">Ticket Details</h3>
 <button onClick={() => setSelected(null)} className="text-muted hover:text-foreground dark:hover:text-white">✕</button>
 </div>
 <div className="glass rounded-2xl p-4 border border-border/10 dark:border-border space-y-2">
 <p className="font-black text-foreground">{selected.subject}</p>
 <p className="text-xs text-muted">{selected.userName} · {selected.userEmail}</p>
 ...
 <div className="bg-secondary/50 dark:bg-card/5 rounded-xl p-4 text-sm text-muted ">
 {selected.message}
 </div>
 <div>
 <label className="text-[10px] font-black text-muted uppercase block mb-2">Resolution / Reply</label>
