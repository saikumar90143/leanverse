...
export default function AdminPage() {
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-50 dark:bg-zinc-950">
        <div className="flex flex-col items-center gap-4">
          <div className="w-10 h-10 border-2 border-emerald-500 border-t-transparent rounded-full animate-spin" />
          <p className="text-sm font-bold text-slate-400">Authenticating...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-50 dark:bg-zinc-950">
        <div className="text-center space-y-3">
          ...
          <p className="font-black text-slate-700 dark:text-zinc-200">Access Denied</p>
          <p className="text-sm text-slate-400">This area requires admin privileges.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-50 dark:bg-zinc-950 flex overflow-hidden">
      {/* Sidebar */}
      <AdminSidebar
