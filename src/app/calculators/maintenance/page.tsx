...
export default function MaintenanceCaloriesCalculator() {
 ...
 {/* Activity Select */}
 <div className="space-y-1">
 <label className="text-xs font-bold text-muted block ml-1">Activity Multiplier</label>
 <select
 value={activity}
 onChange={(e) => setActivity(e.target.value)}
 className="w-full bg-secondary border border-border/20 dark:border-border rounded-xl px-3.5 py-3 text-sm focus:outline-none focus:border-emerald-500 font-medium cursor-pointer"
 >
 ...
 <div className="glass rounded-3xl p-6 sm:p-8 shadow-2xl border border-border/20 dark:border-border text-center relative overflow-hidden flex flex-col items-center justify-center">
 <div className="absolute top-0 right-0 w-24 h-24 bg-cyan-500/5 dark:bg-cyan-500/10 rounded-full blur-2xl -z-10" />

 <span className="text-xs font-black text-muted uppercase tracking-widest block mb-2">Maintenance Target (TDEE)</span>
 ...
 <span key={tdee} className="text-3xl font-black text-foreground transform-gpu">{tdee}</span>
 <span className="text-[10px] uppercase font-extrabold text-muted tracking-wider mt-1">Kcal / Day</span>
 </div>
 </div>

 <div className="w-full grid grid-cols-2 gap-4 border-t border-border/15 dark:border-border pt-6 text-left">
 <div className="p-3 bg-secondary/40 dark:bg-card/5 border border-border/5 rounded-2xl">
 <span className="text-[10px] text-muted uppercase font-black tracking-wider block">BMR Baseline</span>
 <span key={bmr} className="text-lg font-black text-foreground mt-1 block transform-gpu">{bmr} kcal</span>
 </div>
 <div className="p-3 bg-secondary/40 dark:bg-card/5 border border-border/5 rounded-2xl">
 <span className="text-[10px] text-muted uppercase font-black tracking-wider block">Active Burn</span>
 ...
 <div className="glass rounded-2xl p-6 border border-border/10 space-y-4">
 <span className="text-xs font-black text-muted uppercase tracking-widest block">Diet Strategy Baselines</span>
 <div className="space-y-2.5 text-sm font-semibold">
 <div className="flex justify-between items-center py-2 border-b border-border/10">
 <span className="text-rose-500">Fat Loss (-500 kcal)</span>
 <span key={tdee - 500} className="font-extrabold text-foreground transform-gpu">{tdee - 500} kcal/day</span>
 </div>
 <div className="flex justify-between items-center py-2 border-b border-border/10">
 <span className="text-emerald-500">Lean Bulk (+300 kcal)</span>
 <span key={tdee + 300} className="font-extrabold text-foreground transform-gpu">{tdee + 300} kcal/day</span>
 </div>
 <div className="flex justify-between items-center py-2">
 <span className="text-cyan-500">Maintenance (Balanced)</span>
 <span key={tdee} className="font-extrabold text-foreground transform-gpu">{tdee} kcal/day</span>
 </div>
 </div>
 </div>
 </div>
