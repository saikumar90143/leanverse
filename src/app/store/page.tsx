...
export default function SupplementStorePage() {
 ...
 <p className="text-xs text-muted">Discover premium muscle supplements, smart fitness tracking watches, and home gym gear recommended by LeanVerse AI.</p>
 </div>
 ...
 : 'text-muted bg-secondary/50 dark:bg-card/5 hover:text-foreground dark:hover:text-slate-255'
 }`}
 >
 ...
 <button onClick={handleClearCompare} className="text-muted hover:text-red-500 transition-colors">
 <X className="w-4 h-4" />
 </button>
 </div>
 )}
 ...
 {loading ? (
 <div className="col-span-3 text-center py-20 text-muted font-bold">Loading store inventory...</div>
 ) : filteredProducts.length === 0 ? (
 <div className="col-span-3 text-center py-20 text-muted font-bold">No products found. Add some in the Admin Dashboard!</div>
 ) : filteredProducts.map((p) => {
 const isComparing = compareList.some((item) => item.name === p.name);
 return (
 <div key={p.name} className="glass rounded-3xl overflow-hidden border border-border/10 shadow-lg flex flex-col justify-between glow-card relative">
 
 {/* Product Visual */}
 <div className="relative w-full h-48 overflow-hidden bg-foreground flex items-center justify-center p-6">
 <Image 
 src={p.imageUrl} 
 alt={p.name} 
 fill
 ...
 <span key={t} className="absolute top-3 left-3 text-[9px] font-black uppercase tracking-wider bg-foreground/20 text-white px-2 py-0.5 rounded-md backdrop-blur-sm border border-emerald-500/30">
 {t}
 </span>
 ))}
 </div>
 ...
 <span className="text-[10px] text-muted font-extrabold uppercase">{p.category}</span>
 <div className="flex items-center text-amber-400 text-xs font-bold">
 <Star className="w-3.5 h-3.5 fill-current mr-0.5" />
 <span>{p.rating}</span>
 </div>
 </div>
 <h3 className="font-extrabold text-foreground text-sm leading-snug">{p.name}</h3>
 <p className="text-xs text-muted leading-relaxed line-clamp-3">{p.description}</p>
 </div>
 ...
 : 'border-border/10 bg-secondary/50 dark:bg-card/5 text-muted hover:text-cyan-500'
 }`}
 ...
 <div className="glass w-full max-w-3xl rounded-3xl p-6 sm:p-8 border border-border shadow-2xl space-y-6 max-h-[85vh] overflow-y-auto animate-fade-in relative">
 <button
 onClick={() => setCompareModalOpen(false)}
 className="absolute top-4 right-4 p-2 text-muted hover:text-red-500 transition-colors"
 >
 <X className="w-5 h-5" />
 </button>
 ...
 {compareList.map((p) => (
 <div key={p.name} className="p-4 bg-secondary/20 dark:bg-card/5 rounded-2xl border border-slate-350/5 space-y-4">
 <div className="relative h-28 w-full p-2 bg-foreground/20 rounded-xl overflow-hidden">
 <Image src={p.imageUrl} alt={p.name} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-contain p-2" />
 </div>
 <div>
 <h4 className="font-extrabold text-sm text-foreground line-clamp-1">{p.name}</h4>
 <span className="text-[10px] uppercase text-emerald-500 font-extrabold block mt-0.5">{p.category}</span>
 </div>

 <div className="space-y-2 border-t border-border/10 pt-3 text-xs font-semibold text-muted dark:text-slate-350">
 ...
 <div className="pt-2 border-t border-border/10 text-[11px] leading-relaxed">
 {p.description}
 </div>
 </div>
 </div>
