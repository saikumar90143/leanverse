      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute bottom-16 right-0 w-[350px] sm:w-[380px] h-[500px] bg-slate-50 dark:bg-zinc-950 rounded-3xl overflow-hidden flex flex-col shadow-2xl border border-slate-200/50 dark:border-white/10"
          >
            {/* Header */}
            <div className="bg-gradient-to-r from-emerald-500/10 to-cyan-500/10 border-b border-slate-200/20 dark:border-white/10 px-5 py-4 flex items-center justify-between">
              ...
              <button onClick={() => setIsOpen(false)} className="text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar">
                ...
                <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm font-medium leading-relaxed ${
                    msg.sender === 'user'
                      ...
                      : 'bg-slate-200/50 dark:bg-white/5 border border-slate-300/10 dark:border-white/5 text-slate-800 dark:text-slate-200 rounded-bl-none'
                  }`}>
                    <p>{msg.text}</p>
              ...
              {isTyping && (
                <div className="flex justify-start">
                  <div className="bg-slate-200/50 dark:bg-white/5 border border-slate-300/10 dark:border-white/5 text-slate-800 dark:text-slate-200 rounded-2xl rounded-bl-none px-4 py-3 flex items-center space-x-1">
                    <span className="w-1.5 h-1.5 bg-slate-500 dark:bg-slate-400 rounded-full animate-bounce" />
                    <span className="w-1.5 h-1.5 bg-slate-500 dark:bg-slate-400 rounded-full animate-bounce [animation-delay:0.2s]" />
                    <span className="w-1.5 h-1.5 bg-slate-500 dark:bg-slate-400 rounded-full animate-bounce [animation-delay:0.4s]" />
                  </div>
                </div>
              )}
            ...
            {/* Quick Prompts List */}
            {messages.length === 1 && (
              <div className="px-4 py-2 border-t border-slate-200/10 dark:border-white/5 flex flex-wrap gap-1.5">
                {quickPrompts.map((p) => (
                  <button
                    key={p.action}
            ...
            <form
              ...
              className="p-3 bg-slate-100/50 dark:bg-black/20 border-t border-slate-200/15 dark:border-white/5 flex items-center space-x-2"
            >
              <input
                type="text"
                ...
                className="flex-1 bg-white/70 dark:bg-white/5 border border-slate-300/20 dark:border-white/10 rounded-xl px-3.5 py-2 text-slate-800 dark:text-slate-100 text-sm focus:ring-1 focus:ring-emerald-500 focus:outline-none focus:border-emerald-500"
              />
              <button
                type="submit"
